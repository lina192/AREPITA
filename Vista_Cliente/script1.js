...
document.getElementById('miLogin').addEventListener('submit', function(event) {
    ...
    .then(data => {
        if (data.success) {
            window.location.href = '../Vista_Cliente/index.html'; // Redirect if login is successful
        } else {
...
/* Favoritos */ 
    ...
    function addToFavorites(productId) {
        ...
        // Mostrar un mensaje de éxito con botones personalizados
    Swal.fire({
        ...
        customClass: {
            confirmButton: 'custom-btn', // Aplica la clase personalizada al botón de 'Aceptar'
            cancelButton: 'custom-btn-secondary' // Clase personalizada para 'Ver Favoritos'
        }
    }).then((result) => {
